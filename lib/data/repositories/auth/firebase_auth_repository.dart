...
class FirebaseAuthRepository implements AuthRepository {
  ...
  Stream<AuthUser?> get authStateChanges =>
      _firebaseAuth.authStateChanges().map(
            (user) => user != null ? _mapToAuthUser(user) : null,
          );
  ...
  Future<Result<AuthUser>> signInWithEmail({
    ...
    try {
      ...
      return Result.success(_mapToAuthUser(user));
    } on FirebaseAuthException catch (e) {
      AppLogger.error('Sign-in failed', error: e);
      return Result.failed(_mapAuthErrorToMessage(e.code));
    }
  }
  ...
  Future<Result<AuthUser>> registerWithEmail({
    ...
    try {
      ...
      return Result.success(_mapToAuthUser(updatedUser));
    } on FirebaseAuthException catch (e) {
